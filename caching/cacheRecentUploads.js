...
async function getRecentUploads (){
  ...
  for(const category of categories){
    ...
    const recentUploads = await Upload.find(searchQuery).select(selectString).populate('uploader reacts')
      .sort({ createdAt: -1 })
      .limit(1000)
      .lean();
...
async function setRecentUploads (){
  ...
  recentUploads = await Promise.all(recentUploads.map(async (upload, index) => {
  ...
  recentUploads = _.filter(recentUploads, upload => upload.uploader);
