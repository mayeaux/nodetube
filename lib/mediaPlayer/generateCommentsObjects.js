...
async function generateComments(upload) {
  ...
  for (comment of upload.comments) {
    const commentWithResponsesPopulated = await Comment.findOne({ _id: comment.id }).populate({ path: 'responses commenter', populate: { path: 'commenter' } });
  ...
  commentsWithResponsesPopulated = _.filter(commentsWithResponsesPopulated, comment => comment.visibility == 'public' && !comment.inResponseTo);

  // determine if commenter already blocked
  commentsWithResponsesPopulated = commentsWithResponsesPopulated.map((comment) => {
