...
function filterUploadsByMediaType(uploads, mediaType) {
  ...
  if (mediaType == 'image') {
    ...
    return _.filter(uploads, upload => upload.fileType == mediaType);
...
function filterUploadsBySensitivity(uploads, filter) {
  if (filter == 'sensitive') {
    ...
    // return ones not marked as sensitive
    uploads = _.filter(uploads, upload => upload.rating !== 'sensitive');
  } else if (filter == 'allAges') {
    // return ones not marked as sensitive or mature
    uploads = _.filter(uploads, upload => upload.rating !== 'sensitive' && upload.rating !== 'mature');
  }
...
function filterUploadsByCategory(uploads, category) {
  if (category) {
    return _.filter(uploads, upload => upload.category == category);
  }
  return uploads;
}

function filterUploadsBySubcategory(uploads, subcategory) {
  return _.filter(uploads, upload => upload.subcategory == subcategory);
}
...
function trimUploads(uploads, amountToOutput, skipAmount) {
  ...
  const trimmedUploads = uploads.slice(skipAmount);
