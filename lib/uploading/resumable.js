...
module.exports = resumable = function (temporaryFolder) {
  ...
  } catch (e) {console.log('error resumable.js 23');}

  const cleanIdentifier = function (identifier) {
    return identifier.replace(/^0-9A-Za-z_-/img, '');
  };

  const getChunkFilename = function (chunkNumber, identifier) {
  ...
  const validateRequest = function (chunkNumber, chunkSize, totalSize, identifier, filename, fileSize) {
    // Clean up the identifier
    identifier = cleanIdentifier(identifier);
  ...
  $.post = function (req, callback) {
    ...
    if (validation == 'valid') {
      ...
      // saving chunk
      mv(files[$.fileParameterName].path, chunkFilename, (err) => {
