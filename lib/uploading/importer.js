...
async function downloadVideoForUser(channelUrl, youtubeLink){
  ...
  try {
      ...
      // twitter uses a decimal so clear that out by rounding
      let durationInSeconds = Math.round(videoData._duration_raw);
      ...
      // twitter uses a decimal so clear that out, also remove prepended 00 values
      if(formattedDuration) formattedDuration = (formattedDuration.replace(/^(?:00:)?0?/, '')).split('.')[0];
