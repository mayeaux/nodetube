...
async function downloadVideoForUser(channelUrl, youtubeLink){
  ...
  try {
      ...
      // if it's an individual upload, it has a length of two
      // two things of which I'm not exactly sure what they are
      // if there's more than two it's a (sure?) indication that it's a playlist
      if(info.length > 2){
