  ...
  return new Promise(function (resolve, reject) {
      ...
      const command = ffmpeg(uploadedPath).screenshots({
        ...
        if(process.env.NODE_ENV == 'production' && process.env.UPLOAD_TO_B2 == 'true'){
          (async function(){
            const response = await b2.uploadFileAsync(`${saveAndServeFilesDirectory}/${channelUrl}/` + uniqueTag + '.jpeg', {
              name : hostFilePath + '.jpeg',
