...
function convertVideo({ uploadedPath, upload, title, bitrate, savePath, uniqueTag }){
  return new Promise(function(resolve, reject) {
    ...
    let startingTimeInMilliseconds = new Date();
    let lastSpeed = 0
    let averageSpeed = 0
    let percentageFiveSecondsAgo = 0;
    command.on('progress', function (progress) {
      setRedisClient({ progress: progress.percent, uniqueTag  });

      // Update estimated time remaining every five seconds.
      if (new Date() - startingTimeInMilliseconds >= 5000) {
        // Exponential moving average algorithm: https://stackoverflow.com/questions/2779600/how-to-estimate-download-time-remaining-accurately)
        const smoothingFactor = 0.005;
        lastSpeed = (progress.percent - percentageFiveSecondsAgo) / 5;
        averageSpeed = (!averageSpeed && lastSpeed) || smoothingFactor * lastSpeed + (1-smoothingFactor) * averageSpeed;

        const estimatedTimeRemaining = (100 - progress.percent) / averageSpeed;
        upload.estimatedTimeRemaining = estimatedTimeRemaining;

        percentageFiveSecondsAgo = progress.percent;
        startingTimeInMilliseconds = new Date();
      }
      client.get(`${uniqueTag}uploadProgress`, function(err, value) {
        if (err) {
      ...
      console.log(`${title}: CONVERTED: ${progress.percent}`, progress);
