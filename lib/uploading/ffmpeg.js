...
function getSecondsRemaining(currentProgress, secondsSinceStarting, remaining = 100){
  ...
  console.log("Types", typeof currentProgress, typeof secondsSinceStarting)
  console.log(currentProgress, secondsSinceStarting)
...
function setRedisClient({ currentSeconds, uniqueTag, progress, force }){
  ...
  const formattedTimeLeft = timeHelper.secondsToFormattedTime(secondsRemaining);
  ...
  if(force == true){
    redisClient.setAsync(`${uniqueTag}timeLeft`, formattedTimeLeft)
