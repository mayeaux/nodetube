...
function getSecondsRemaining(currentProgress, secondsSinceStarting){
...
function setRedisClient({ currentSeconds, uniqueTag, progress, force }){
  ...
  //
  // // I think this will work (reusing the function used for getting the duration
  // const formattedTimeLeft = useYourDurationFunctionThing()

  // get number of seconds (0-60)
  const currentSecond = new Date().getSeconds();

  // only update redis every third second just in case it's over a network

  if(force == true){
    // TODO: try this here, let's set a TTL on it as well (so that it expires after a certain time)
    // redisClient.setAsync(`${uniqueTag}timeLeft`, Math.ceil(formattedTimeLeft))

    // TODO: the rest you can follow what is done for uploadProgress and you can reverse engineer it and implement it for time remaining
