...
async function updateUsersEmailNotifications(user, upload){
  ...
  console.log("updateUsersEmailNotifications -> emailSubscriptions", emailSubscriptions)
  ...
  for(const sub of emailSubscriptions){
    ...
    const mailOptions = {
      to: sub.subscribingUser.email,
      subject: `New upload by: ${user.channelUrl}: ${upload.title}`,
      body: `
      ${user.channelUrl} has uploaded a new video: ${upload.title}.
        <button>Watch now!</button>
        <img src='/uploads/${user.channelUrl}/${upload.uniqueTag}.jpg' alt='Video preview' width='500' height='600'>
      `
    };
